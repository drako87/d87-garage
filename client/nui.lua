    ...
    lib.callback('d87-garage:server:spawnVehicle', false, function(success, result)
        if success then
            ...
            if veh and veh > 0 then
                ...
                Bridge.Notify(locale('error.spawn_failed') or 'Spawn failed locally', 'error')
            end
        else
            Bridge.Notify(result or locale('error.spawn_failed') or 'Error spawning vehicle', 'error')
        end
